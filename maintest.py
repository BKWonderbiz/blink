...
from blink20 import BlinkDetector
import asyncio
...
def detect_known_faces(known_face_id, known_face_names, known_face_encodings, frame, conn):
    ...
    for i, face_encoding in enumerate(face_encodings):
        ...
        if matches[best_match_index] and face_distances[best_match_index] < 0.45:
            ...
            if is_real and face_distances[best_match_index] < 0.4:
...
#         blink_count, processed_frame = blink_detector.detect_blink(frame)

#         # Start recognition if a blink is detected
#         if blink_count > 0 and not recognition_active:
#             recognition_active = True
#             face_locations, face_names = detect_known_faces(known_face_id, known_face_names, known_face_encodings, processed_frame, conn)

#             for (top, right, bottom, left), name in zip(face_locations, face_names):
#                 cv2.rectangle(processed_frame, (left, top), (right, bottom), (0, 0, 255), 2)
#                 cv2.putText(processed_frame, name, (left, top - 10), cv2.FONT_HERSHEY_SIMPLEX, 0.75, (0, 0, 255), 2)

#             # After processing the recognition, deactivate until the next blink
#             recognition_active = False
...
async def websocket_endpoint(websocket: WebSocket):
    ...
    while True:
        ...
        blink_detected, processed_frame = blink_detector.detect_blink(frame)

        # If a blink is detected, process the frame
        if blink_detected and not recognition_active:
            recognition_active = True  # Start recognition
            ...
            recognition_active = False  # Stop recognition until next blink
